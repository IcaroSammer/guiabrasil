class Admin::TypeAttractionsController < Admin::BaseController
  ...
  def create
    ...
    if @type_attraction.save
      redirect_to admin_type_attractions_path, notice: "Registro #{@type_attraction.name} Criado com sucesso"
    else
  ...
  def update
    if @type_attraction.update(type_attraction_params)
      redirect_to admin_type_attractions_path, notice: 'Atualizado com sucesso'
    else
      render action: :edit
    end
  end
  ...
  def destroy
    @type_attraction.active =  @type_attraction.active ? false : true
    if @type_attraction.save
       redirect_to admin_type_attractions_path, notice:  @type_attraction.active  ? 'Registro ativado com sucesso' : 'Registro desativado com sucesso'
    end
  end
  def filters
